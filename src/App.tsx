...
const App: React.FC = () => {
  ...
  const [pageData, setPageData] = useState<{
    forYou: { page: number; hasMore: boolean };
    following: { page: number; hasMore: boolean };
  }>({
  ...
  useEffect(() => {
    ...
    // Seguir automáticamente a los primeros 5 usuarios
    const initialFollowed = new Set(users.slice(0, 5).map(u => u.id));
    setFollowedUsers(initialFollowed);
  }, []);

  const generateTweets = (users: User[], count: number): Tweet[] => {
    return Array.from({ length: count }, (_, i) => {
      ...
      return {
        id: `tweet-${Date.now()}-${i}`,
        userId: user.id,
        content: `Este es un tweet de @${user.handle}. ${hasImage ? '' : 'Contenido de ejemplo para demostrar el funcionamiento del feed.'}`,
        likes: Math.floor(Math.random() * 1000),
        retweets: Math.floor(Math.random() * 500),
        replies: Math.floor(Math.random() * 300),
        timeAgo: `${Math.floor(Math.random() * 60)}m`,
        images: hasImage ? [
          `https://picsum.photos/seed/${Date.now()}-${i}a/600/400`,
          `https://picsum.photos/seed/${Date.now()}-${i}b/600/400`
        ].slice(0, Math.floor(Math.random() * 2) + 1) : undefined
      };
    });
  };

  const fetchTweets = useCallback(async (tab: typeof activeTab) => {
    ...
    try {
      ...
      const newTweets = generateTweets(allUsers, 10);

      if (tab === 'following') {
        const filteredTweets = newTweets.filter(t => followedUsers.has(t.userId));
        setFollowingTweets(prev => [...prev, ...filteredTweets]);
  ...
  }, [loading, pageData, followedUsers, allUsers, activeTab]);
                ...
                <div className="tweet-content">
                  <div className="tweet-header">
                    <div className="tweet-user">
                      ...
                      {activeTab === 'forYou' && followedUsers.has(user.id) && (
